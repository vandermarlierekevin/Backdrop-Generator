...
If images are attached without a settings block, use sensible defaults (match focal length, match f-stop, circular bokeh, no specific environment) and generate immediately.
...
- Estimated f-stop (from depth of field characteristics and shadow edge quality)
- Camera height (low, eye level, slightly above/below, high)
- Camera angle (straight-on, slight upward, slight downward)
- Bokeh character visible (if any)

Write as one concise paragraph.

---

## Step 2 — F-stop rendering guidance

Use the f-stop setting to determine how depth is described in the prompt:

- **f/1.2 – f/1.8**: Extremely shallow depth of field, razor-thin focus plane, heavy background separation, creamy blur
- **f/2 – f/2.8**: Shallow depth of field, background clearly out of focus, subject isolated
- **f/4**: Moderate depth of field, background softened but not eliminated
- **f/5.6**: Near-deep depth of field, background detail visible but slightly soft
- **f/8 – f/11**: Deep depth of field, sharp front to back
- **f/16 – f/22**: Maximum depth, everything sharp, slight diffraction softness
- **Match shot**: Estimate from the green screen image and describe accordingly

Always include the f-stop value explicitly in the prompt text (e.g. "shot at f/2.8").

---

## Step 3 — Bokeh type guidance

The bokeh type setting describes the *character* of the out-of-focus rendering — independent of how much blur there is. Always include bokeh type in prompts even when depth of field is deep (it affects how any specular highlights render). Use these descriptions:

- **Circular**: smooth, perfectly round bokeh balls
- **Swirly**: vintage Helios-style swirling, turbulent bokeh
- **Cat eye**: oval bokeh toward frame edges, rounder in centre (fast prime characteristic)
- **Anamorphic oval**: horizontally stretched oval bokeh with horizontal lens flare streaks
- **Soap bubble**: bright ring-outlined bokeh balls with defined edges (Trioplan/Biotar style)
- **Geometric**: polygonal bokeh shapes from aperture blades (5-, 6-, or 8-blade aperture)
- **Nervous / busy**: chaotic, textured, high-contrast bokeh with complex highlight shapes
- **Soft / feathered**: no defined shape, purely soft luminous glow, painterly
...
- Match shot analysis exactly: camera height, angle, focal length, f-stop, lighting direction, color temp, shadow quality
- Apply all settings from the `[BACKDROP_SETTINGS]` block: environment, mood, colorTemp, focal, fstop, bokehType, objects, notes
- Include the specific bokeh type character in the description of any background highlights or out-of-focus elements
...
Always specific: "soft key light from the upper left at 45°", "shadows falling to the lower right", "worn oak hardwood floor", "exposed concrete", "thin wisps of smoke drifting through amber light". Specify focal length, f-stop, and bokeh character explicitly. Never vague ("dark moody background" is unacceptable).
...
Render this artifact at conversation start and when user types "new". No external font loads. No image upload controls — images go in the chat. The Generate button reads all current settings and sends them to Claude via sendPrompt().
...
<div class="sec" style="margin-top:8px">
  <div class="sec-label">F-Stop</div>
  <div class="cg">
    <div class="cw" id="ac">
      <div class="chip so" onclick="sc(this,'a','match')">Match my shot</div>
      <div class="chip" onclick="sc(this,'a','1.2')">f/1.2</div>
      <div class="chip" onclick="sc(this,'a','1.4')">f/1.4</div>
      <div class="chip" onclick="sc(this,'a','1.8')">f/1.8</div>
      <div class="chip" onclick="sc(this,'a','2.8')">f/2.8</div>
      <div class="chip" onclick="sc(this,'a','4')">f/4</div>
      <div class="chip" onclick="sc(this,'a','5.6')">f/5.6</div>
      <div class="chip" onclick="sc(this,'a','8')">f/8</div>
      <div class="chip" onclick="sc(this,'a','11')">f/11</div>
    </div>
  </div>
</div>

<div class="sec" style="margin-top:8px">
  <div class="sec-label">Bokeh type</div>
  <div class="cg">
    <div class="cw" id="bc">
      <div class="chip so" onclick="sc(this,'b','circular')">Circular</div>
      <div class="chip" onclick="sc(this,'b','swirly')">Swirly</div>
      <div class="chip" onclick="sc(this,'b','cats-eye')">Cat eye</div>
      <div class="chip" onclick="sc(this,'b','anamorphic')">Anamorphic oval</div>
      <div class="chip" onclick="sc(this,'b','soap-bubble')">Soap bubble</div>
      <div class="chip" onclick="sc(this,'b','geometric')">Geometric</div>
      <div class="chip" onclick="sc(this,'b','nervous')">Nervous / busy</div>
      <div class="chip" onclick="sc(this,'b','soft')">Soft / feathered</div>
    </div>
  </div>
</div>
...
<script>
let rlOn=false,selL=null,selF='match',selA='match',selB='circular';
...
const ad={match:'f-stop matching the depth of field estimated from the reference shot','1.2':'f/1.2 — razor-thin focus plane, extreme background separation, very heavy blur','1.4':'f/1.4 — very shallow depth of field, strong background separation','1.8':'f/1.8 — shallow depth of field, subject clearly isolated from background','2.8':'f/2.8 — shallow depth of field, background softened and out of focus','4':'f/4 — moderate depth of field, background visibly soft','5.6':'f/5.6 — near-deep depth of field, background detail present but slightly soft','8':'f/8 — deep depth of field, sharp front to back','11':'f/11 — maximum depth of field, everything critically sharp'};
const bd={circular:'smooth perfectly round circular bokeh balls',swirly:'vintage Helios-style swirling turbulent bokeh with a spiraling character','cats-eye':"cat's eye oval bokeh toward the frame edges, rounder toward centre, fast prime character",anamorphic:'horizontally stretched anamorphic oval bokeh with horizontal lens flare streaks','soap-bubble':'soap bubble bokeh with bright ring-outlined bokeh balls and defined edges, Trioplan style',geometric:'polygonal geometric bokeh shapes from aperture blades',nervous:'chaotic nervous busy bokeh with complex high-contrast highlight shapes',soft:'soft feathered bokeh with no defined shape, purely luminous painterly glow'};
...
function sc(el,g,v){document.querySelectorAll('#'+g+'c .chip').forEach(c=>c.classList.remove('sb','so'));const isDefault=(g==='f'&&v==='match')||(g==='a'&&v==='match')||(g==='b'&&v==='circular');el.classList.add(isDefault?'so':'sb');if(g==='f')selF=v;if(g==='a')selA=v;if(g==='b')selB=v}
function go(){
